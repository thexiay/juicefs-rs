...
use bytes::Bytes;
use juice_meta::{
    api::{new_client, Falloc, Meta, OFlag},
...
async fn new_vfs() -> Vfs {
    ...
    let operator = Operator::new(Memory::default().root("/tmp")).unwrap().finish();
...
async fn test_vfs_io() {
    ...
    let (fh, entry) = vfs.create(1, "file", 0o755, 0, OFlag::O_RDWR).await.expect("create file:");
    vfs.fallocate(entry.inode, Falloc::NONE, 0, 64 << 10, fh).await.expect("fallocate:");
    vfs.write(entry.inode, 0, fh, Bytes::from_static(b"hello")).await.expect("write file:");
    vfs.fsync(entry.inode, fh).await.expect("fsync file:");
    vfs.write(entry.inode, 100 << 20, fh, Bytes::from_static(b"world")).await.expect("write file:");

    let attr = vfs.truncate(entry.inode, (100 << 20) + 2, fh)
        .await
